...
export class STM32F4 {
  ...
  // Peripheral-domain helpers (ETH status, DMA2D jobs, NAND, trace/FPU).
  ethMacAddr(): bigint;
  ethTxDeferred(): boolean;
  ethCollisionArmed(): boolean;
  ethArmCollision(): void;
  ethPauseTx(): number;
  ethBackoffSlots(attempt: number, seed: number): number;
  dma2dTakeJob(): number[];
  dma2dJobDone(): void;
  dma2dFifoThreshold(dma: string, stream: number): number;
  dmaStreamCt(dma: string, stream: number): boolean;
  dmaStreamFeif(dma: string, stream: number): boolean;
  fsmcBindNand(bank: number, size: number): void;
  fsmcNandErase(bank: number, offset: number, len: number): void;
  traceStart(): void;
  traceStop(): void;
  takeTrace(): number[];
  getFpuState(): any;
  ...
  wdogRequested(): boolean;
  wdogClearFlags(): void;
  wdogIwdgFlag(): boolean;
  wdogWwdgFlag(): boolean;
  flashProgramming(): boolean;
  // Protocol helpers (USB status/VBUS, UART LIN/SMARTCARD/IrDA, CAN error/FD).
  usbInStatus(ep: number): number;
  usbOutStatus(ep: number): number;
  usbHsInStatus(ep: number): number;
  usbHsOutStatus(ep: number): number;
  usbSetVbus(present: boolean): void;
  usbHsSetVbus(present: boolean): void;
  usbFrame(): number;
  usbHsFrame(): number;
  usbUlpiRate(): number;
  usbHsUlpiRate(): number;
  usbDmaProgress(ep: number): any;
  uartBreakTx(base: number): void;
  uartIrdaRx(base: number, byte: number, lowPower?: boolean): void;
  uartIrdaTxClass(base: number): number;
  uartSmartcardNack(base: number): void;
  uartSmartcardRetries(base: number): number;
  canNoteError(base: number, lec: number, recover?: boolean): void;
  canFdLen(base: number, fifo: number, slot: number): number;
  canFdByte(base: number, fifo: number, slot: number, idx: number): number;
  canFdCost(base: number, fd: boolean, brs: boolean, payloadBytes: number): bigint;
  i2cPec(base: number): number;
  spiFlashDebug(peripheral: string): number[];
  sdioCardIrq(set: boolean): void;
  rtcTamper(): void;
  audioLoadWav(bytes: Uint8Array | number[]): void;
  ltdcClutEntry(layer: number, idx: number): number;
  ltdcLutPixel(layer: number, pf: number, byte: number): number;
