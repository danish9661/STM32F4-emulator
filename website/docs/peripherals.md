...
|---|---|---|---|---|---|---|
| ADC | 881 | Detailed | SR/CR1/CR2/SMPR1-2/JOFR/HTR/LTR/SQR1-3/JSQR/JDR/DR; SWSTART-gated conversion (edge-triggered; CONT re-anchors for continuous — mock `t_gap9`), ALIGN left-shifts DR by 4 (mock `t_gap9`), real sampling-time logic (SMPR lookup), channel values (16/17 = temp/Vref, 18 = Vbat, others LCG pseudo-random), EOC/OVR flags (overrun: new conversion while EOC set latches OVR, DR read clears the triple EOC/OVR/AWD — mock `t_honor_pass`); injected group via JSWSTART edge/JAUTO (JL/JOFR/JDR/JEOC/JSTRT, JEOCIE IRQ — mock `t_gap9`); analog watchdog (AWDEN vs JAWDEN group-gated + AWDSGL/AWDCH + HTR/LTR compare, AWD flag + IRQ on excursion — mock `t_adc_awd_dma`, group gate `t_gap9`); DMA request on EOC (CR2 DMA bit stages samples for `adc_take_dma` — mock `t_adc_awd_dma`); shared **ADC_Common @ 0x40012300** — CSR mirrors EOC1/2/3+OVR1/2/3 (read-only), CDR = `ADC1.DR\|ADC2.DR<<16`, CCR DUAL mode + CDR simultaneous latch (both-ready peek, `adc_dual_latched` probe — mock `t_adc_dual`); `C_ADC` Keil alias unified so the F429 map gets the block | Y (EOC/OVR/AWD → 18/47) | N | N |
...
| DAC | 256 | Detailed | CR/SWTRIGR/all DHR regs/DOR1-2/SR; TSEL trigger mux (TIM6/TIM8/TIM7/TIM5/TIM2/TIM4/EXTI9/SW, TEN-gated) + DMAUDR underrun latch + w1c (`dac_hw_trigger`/`dac_underrun` — mock `t_gap10`); DOR update on trigger + writes (register model real — mock `t_dac_sink`), LFSR noise, triangle-waveform counter/direction, MAMP masks; no analog pin layer (DOR readback IS the sink value + `sink_sample` probe — mock `t_dac_sink`) | N | N | N |
| DBGMCU | 225 | Detailed | IDCODE per-map (untouched F407 map reads 0x10006411 — the shipped firmware probe constant; F401 0x423/F411 0x431/F429 0x419/0x413 via `init_svd_chip` + `set_idcode`, DEV_ID[11:0] exact, REV_ID 0x1000 — mock `t_dbgmcu_idcode`; native `idcode_per_map_and_cr_mask` test); CR/APB1FZ/APB2FZ stored/masked (CR mask 0x1F_E0F7 — keeps the 0x70 nibble the shipped probes round-trip); freeze HONORED — `dbgmcu_set_halt()` + halted/frozen TIM2-14/WWDG/IWDG neither count nor fire, I2C START/STOP sequencing holds (SMBUS-timeout bits 21-23), no catch-up burst on resume; name alias unified (`DBGMCU` vendor-SVD + `DBG` Keil DFP) so every map gets the block; cargo `dbgmcu_freeze_*` tests | N | N | N |
...
| DMA | 452 | Detailed | LISR/HISR/IFCR + 8 streams (CR/NDTR/PAR/M0AR/M1AR/FCR + DBM/CT target latch); FCR FIFO thresholds (FTH 1/2/3/4 words) + burst-vs-threshold FEIF stall + DBM-direct TEIF config error (`dma_stream_feif/ct/fifo_threshold` probes — mock `t_gap10`); EN queues a `DmaTransfer` (copy done by JS via `dma_get_pending`/`dma_set_completed`), TCIF/HTIF, stream IRQ, double-buffer M1AR, dir/mem2mem | Y (DMA1: 11-18, DMA2: 56-63) | N | N |
...
| ETH | 2713 | Detailed | All 4 blocks (MAC/MMC/PTP/DMA) full register maps incl. WOL wakeup-filter CRC, addend drift, PPS counter/level, VLAN gate+invert, PTP snapshots, TX checksum insert, RX checksum status, accept filtering (perfect/hash/promiscuous/DAIF/ROD), wire pacing, deferral/collision reports, RBUS head-only RX, EDFE + TCH/TER/RCH/RER chain walks + RDES4 extended status + backoff slot probe (mock `t_gap11`); DP83848-style PHY/MDIO peer; MII/RMII pin mirrors in GPIO IDR; BSD sockets over real LwIP; PPS pin live on PB5 IDR mirror (same advance-then-read path as the counter — mock `t_pps`), nibble data + 25/50 MHz clocks electrical-only (level mirrors are the contract — mock `t_nibble`); ULPI rate report 480/12 in GUSBCFG + scope probes (mock `t_ulpi_rate`); guest coverage: `eth_feat_test` (60+ markers, both maps), `eth_http/dhcp/test/irq/pins`, `lwip_demo`, `eth_adv` (12 phases: RST/RTO/MSS/window/frag/ICMP/DHCP-NAK/renew/IGMP/ND/LLDP/STP, both maps) | Y (61/62) | Y | Y |
...
| GPIO | 407 | Detailed | MODER/OTYPER/OSPEEDR/PUPDR/IDR/ODR/BSRR/LCKR/AFRL/AFRH; ODR/BSRR drive output_state + write callbacks, MODER→input clears output, IDR = input_state + read callbacks, 11 ports (A-K); LCKR key sequence (`W(1<<16+LCKk)→W(LCKk)→W(1<<16+LCKk)→R` confirms LCKK) + per-pin config freeze of MODER/OTYPER/OSPEEDR/PUPDR/AFRL/AFRH under engaged LCKK (mock `t_honor_pass`) | N | N | Y |
...
| I2C | 821 | Detailed | Full master state machine (Idle→Start→Addr→Active read/write) + slave mode (OAR1/OAR2 own-address match → ADDR (OR-latched, never wipes RXNE) → DR rx-drain/tx-stage incl. pre-address staging → STOP; `i2c_slave_address/write/read/stop/status` harness — mock `t_gap10`; guest coverage: `gap10_i2c` (+ in-browser CDP smoke)), START/STOP, SWRST, address match vs attached EEPROMs (NACK/AF on miss), byte R/W, SR1/SR2 ordering semantics, event/buffer/error IRQ masking; SMBus PEC (CRC-8, ENPEC-gated, PECERR — mock `t_i2c_smbus`), GCALL/ALERT/ARP model answers (GENCALL/SMBALERT, host-notify via `i2c_arm_smbus_alert` — mock `t_smbus_addr`), ARLO on armed loss (harness = other master), TIMEOUT after 32 Active ticks with SMBUS set; single-master only otherwise (bus always won — mock `t_i2c_multi`) | Y (31-34, 72-73) | Y | Y |
...
| LTDC | 382 | Detailed | Global regs + 2 layers (CR/whpcr/wvpcr/ckcr/pfcr/cacr/dccr/bfcr/cfbar/cfblr/cfblnr/clutwr); real scanline/frame advance from SSCR/BPCR/AWCR geometry (`ltdc_get_scanline`/`ltdc_get_frame_count`), LIF at LIPCR + frame-end F flag, IRQ 88; CLUT load with auto-increment + L8/AL44/AL88 LUT-indexed resolve (`ltdc_clut_entry`/`ltdc_lut_pixel` probes — mock `t_gap10`); browser console renders layer0 (ARGB8888/RGB565) to a canvas; guest coverage: `gap10_ltdc` (CLUT pointer) | Y (88) | Y | N |
...
| PWR | 276 | Detailed | CR/CSR masked storage (incl. F429 VOS/ODEN/ODSWEN/UDEN family fields), WUF/SBF via emulator wakeup path (CSR bit 0/1, cleared via CR CWUF/CSBF); PVD live (PVDO follows PVDE, pinned-healthy rail); VOS settle window (~1000 inst); F429 overdrive/under-drive handshake (ODEN→ODRDY, ODSWEN→ODSWRDY, UDEN→UDRDY after ~5000-inst windows, ready drops with enable — mock `t_pwr`) | N | N | N |
| QSPI | 515 | Detailed | QUADSPI @0xA0001000 (absent from the F407 SVD, registered explicitly at its conventional base): indirect read/write to a `qspi_register_flash()` image bound before init (mirrors the SPI/FSMC tap pattern); memory-mapped window (FMODE=11 → AHB 0x90000000 reads live from the image, survives indirect completions, `qspi_mmap_live`/`qspi_mmap_read` probes — mock `t_gap10`); 32-byte data FIFO, BUSY/TC/FT flags; guest coverage: `qspi_test` (+ in-browser CDP smoke), `gap10_qspi` (+ in-browser CDP smoke) | N | N | Y |
...
| RTC | 539 | Detailed | TR/DR/CR/ISR/PRER/WUTR/CALIBR/ALRMAR/ALRMBR/WPR/SSR/SHIFTR/timestamp/CALR/TAFCR/ALRMASSR/ALRMBSSR + 20 backup regs; BCD time advances via PRER prescaler vs instruction count, alarm A/B matching with don't-care masks, wakeup timer (WUTR reload + WUTE countdown latches WUTF + IRQ 2 — mock `t_rtc_wut_ts`), SHIFTR sub-second shift (SUBFS borrow/ADD1S/RSF — mock `t_gap9`), ALRMASSR/ALRMBSSR MASKSS sub-second alarm gate (unprogrammed registers do not gate — mock `t_gap9`), timestamp capture + overrun (TSF/TSOVF — mock `t_rtc_wut_ts`), tamper-pin physics (TAMP1E/TRG/FLT/PRCH/FREQ, BKPR erase, TAMPTS — mock `t_rtc_tamper_phys`), smooth calibration (CALR CALM/CALP rate — mock `t_rtc_wut_ts`) | Y (41) | N (advances on access) | N |
...
| SDIO | 566 | Detailed | Full register set + emulated SD card state machine (Idle→Ident→Stby→Tran), canned responses for CMD0/2/3/5/7/8/9/10/13/16/17/18/24/41/55, RCA matching, CMD24 single-block write (FIFO TX staging commits into the bound `sdio_bind_card` image, CMD17 round-trips; post-DATAEND FIFO retains words for late draining, direction latched at CMD — mock `t_gap10`); guest coverage: `gap10_sdio` (+ in-browser CDP smoke), ACMD prefix tracking (CMD55 latches APP_CMD; ACMD41 answers OCR-ready only under prefix, ACMD6 latches WIDBUS 1/4/8-bit, ACMD13/51 canned; bare ACMDs illegal — mock `t_sdio_acmd`), DAT1 card IRQ (SDIOIT + IRQ49, mock `t_sdio_acmd`), width-scaled data timing (4-bit beats 1-bit; DTIMER→DTIMEOUT, DCRCFAIL fault, RXOVERR/TXUNDERR on zero-length — mock `t_sdio_timing`), data-transfer simulation (DCOUNT/FIFOCNT, CMD17/18), status flags + ICR/MASK | Y (49) | N | N |
...
| TIM | 1012 | Detailed | CR1/CR2/SMCR/DIER/SR/EGR/CCMR1-3/CCER/CNT/PSC/ARR/CCR1-6/RCR/DCR/DMAR/OR/BDTR; instruction-count counter with PSC prescaler, up/down/center-aligned, UIF+UIE IRQ on overflow, CC match IRQs, UG update event, PWM duty (MOE-gated on TIM1/TIM8); BDTR/MOE/break (BKE/BIF/BIE/AOE/LOCK — mock `t_gap9`); RCR repetition gating (mock `t_gap9`); TIF/COMIF + EGR software events CCxG/TG/COMG/BG (EGR reads 0 — mock `t_gap9`); OPM one-pulse (CEN self-clears at update in every counting mode — mock `t_honor_pass`); master/slave trigger routing (CR2 MMS reset/update → ITR slaves SMS reset/gated/trigger, deferred end-of-tick drain — mock `t_trgo`); DBGMCU freeze stops advance while halted+frozen; encoder counting live via `tim_encoder_step` (SMS/polarity/wrap — mock `t_tim_enc`) | Y (per-timer) | Y | N |
| USART | 527 | Detailed | SR/DR/BRR/CR1-3/GTPR; TX pushes to global UART output buffer (drained by `get_uart_output`), RX via `rx_byte` (JS `uart_rx_byte`) with 64-byte RX buffer, RXNE/ORE/IDLE (harness `uart_idle`, IDLEIE IRQ, DR-read clear), mute mode (RWU drops, WAKE=0 idle exit, WAKE=1 mark exit + delivery, `uart_muted` probe — mock `t_gap9`), TXE/TC always set; TX break via SBK (queued flag, self-clearing CR1 bit, consumed by DR write, `uart_break_pending` probe); IRQs on RXNEIE/TCIE/TXEIE/IDLEIE/PEIE/LBDIE (PE is PEIE-gated, not EIE — mock `t_honor_pass`); HW flow control (CTSE gates TX on harness CTS, TXE/TC clear while held) + FE/PE fault injection (PCE-gated PE, DR-read clear) (mock `t_usart_flow_err`); LIN break (LBD + RXNE/0x00, LBDIE IRQ — mock `t_usart_protocols`), Smartcard T=0 NACK retry loop (TC held, 8-cap, NE on exhaustion — mock `t_usart_protocols`), IrDA pulse-class envelope (NE on mismatch — mock `t_usart_protocols`); LIN/SC/IrDA enable bits stored + probed (`lin_active`/`sc_active`/`irda_active`), async path unchanged (mock `t_usart_modes`) | Y (37/38/39/52/53/71/82/83) | N | N |
| USB | 1229 | Detailed | OTG FS device @0x50000000 + OTG HS in FS mode @0x40040000 (IRQ 77, HS ENUMSPD, own window — same device core; the SVD OTG_FS_*/OTG_HS_* sub-entries stay dropped): GINTSTS+GINTMSK (USBRST/ENUMDNE/RXFLVL/NPTXFE/IEPINT/OEPINT, W1C), GRXSTSP(pop)/GRXSTSR(peek), device block, EP CTL/INT(XFRC+EPDISD, W1C)/TSIZ/DMA/DTXFSTS-from-programmed-FSIZ; IN completes synchronously at EPENA, OUT on short-packet/drained XFRSIZ, STALL handshake per-EP (CTL bit 21, `usb_in_status`/`usb_out_status` — mock `t_stall`); IRQ67 when masked+live; host-driven via `usb_*` / `usb_hs_*` exports (`usbhost.js` `{hs:true}` in-page, `test_usb.mjs` + matrix `usb` script; HS e2e = full CDC enum + 2x echo + STALL on the HS block — mock `t_usbhs`); SOF 1 kHz while enumerated (W1C, FNSOF — mock `t_usb_sof`), HS 8x microframes + EOPF + `usb_hs_uframe` (mock `t_hs_uframe`), ULPI rate report 480/12 in GUSBCFG + scope probes (mock `t_ulpi_rate`), VBUS sense via `usb_set_vbus` (BSVLD/SEDET/SUSPSTS — mock `t_usb_vbus`), internal-DMA accounting via DMAEN + `usb_dma_progress` (mock `t_usb_dma`); no isochronous/host (HPTXFSIZ=0), no suspend-resume IRQ path beyond RWUSIG (mock `t_usb_gaps`) | Y (67/77) | Y | Y |
...
- **SPI + FSMC + SDIO + DCMI + LTDC + I2C + CAN + ADC + DAC + RNG + RTC**:
  `spi_tft_test`, `comprehensive_test`, `edge_test`, `hardware/saturn`+`hardware/monox`
